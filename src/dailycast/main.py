...
def create_app(*, config_path: Path | None = None) -> FastAPI:
    ...
    @app.post("/briefing/dry-run", status_code=202, tags=["briefing"])
    async def dry_run_briefing(
        runtime: RuntimeDependency,
        authorization: Annotated[str | None, Header(alias="Authorization")] = None,
    ) -> JSONResponse:
        """Run one full briefing generation in the background without sending it.
        ...
        and output files land below the dry-run subdirectory. The run outlives
        this request on purpose: production gateways cut long connections, so
        the outcome is persisted as a JSON report readable through
        ``GET /briefing/dry-run``. On a public_only deployment the route
        requires the same bearer token as the other operator routes.
        """
        if runtime.settings.app.public_only:
            _require_manual_trigger_token(runtime, authorization)
        _require_ready(runtime)
        if runtime.briefing_service is None:
            raise HTTPException(status_code=409, detail="briefing is not enabled")
        try:
            task = runtime.briefing_service.create_dry_run_task()
        except BriefingRunInProgressError:
            raise HTTPException(
                status_code=409, detail="briefing run already in progress"
            ) from None
    ...
    @app.get("/briefing/dry-run", tags=["briefing"])
    async def latest_briefing_dry_run(
        runtime: RuntimeDependency,
        authorization: Annotated[str | None, Header(alias="Authorization")] = None,
    ) -> JSONResponse:
        """Return the most recent persisted dry-run report for acceptance checks.
        ...
        if runtime.briefing_service is None:
            raise HTTPException(status_code=409, detail="briefing is not enabled")
        report = runtime.briefing_service.latest_dry_run_report()
        if report is None:
            raise HTTPException(status_code=404, detail="no dry run has finished yet")
