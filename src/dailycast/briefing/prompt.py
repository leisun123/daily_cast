...
def _editorial_selection_instruction(category: str | None, *, required_count: int) -> str:
    ...
    shared = (
        ...
        "只选有明确主体、动作、结果或下一步的实质性报道。同一事件被多家独立来源报道说明事件关注度高："
        "必须合并为一条入选（选事实最完整、来源最可靠的一篇作代表，不得逐条罗列），"
        "绝不能因为'多个来源都在报道同一件事'就把该事件整体跳过；只有同一来源对同一事件的"
        "重复推送才需要去重。候选中有足够合格内容时必须选满 "
        f"{required_count} 条，并尽量覆盖不同来源。"
    )
    if category == "telecom":
        return (
    ...
    if category == "ai":
        return (
            ...
            "市场热度的热门应用与热点、智能体和具身智能。上述头部厂商的重大模型发布、重大版本"
            "升级或重量级开源属于必选头条事件：候选池中只要存在此类报道（哪怕多家报道同一事件），"
            "就必须合并选入一条代表并排在最前；该报道发布时间早于窗口内其他候选不是遗漏的理由。"
            "其余条目在候选充足时，模型与部署优先 2 条，"
