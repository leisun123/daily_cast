...
YAML 只引用环境变量名，例如主模型使用 `DAILYCAST_LLM__API_KEY`、备用模型使用 `DAILYCAST_LLM__FALLBACK__API_KEY`。应用读取值后只将 Secret 类型传给 Provider；序列化、repr 和日志过滤器必须脱敏。`.env.example` 只列变量名和说明，不含任何真实值。生产部署优先使用 Compose secret/environment 注入并限制 `.env` 权限。
