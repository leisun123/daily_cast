...
def test_publication_targets_migration_projects_existing_rss_publications(
    app_config_path: Path,
) -> None:
    """Existing atomic RSS Publication rows gain exactly one matching target lifecycle row."""
    settings = load_settings(config_path=app_config_path)
    ini_path = Path(__file__).resolve().parents[1] / "alembic.ini"
    config = build_alembic_config(ini_path=ini_path, database_url=settings.database.url)
    command.upgrade(config, "0006_backfill_episode_news_count")
    ...
    try:
        with engine.begin() as connection:
            connection.execute(
                text(
                    ...
                    INSERT INTO episodes (
                        public_id, episode_date, edition, status, script_revision, audio_version,
                        lock_version, created_at, updated_at
                    ) VALUES (
                        'target-backfill-episode', '2026-07-24', 'daily', 'published', 1, 1,
            ...
            episode_id = connection.execute(
                text("SELECT id FROM episodes WHERE public_id = 'target-backfill-episode'")
            ).scalar_one()
            connection.execute(
                text(
                    """
                    INSERT INTO publications (
                        episode_id, publisher_type, target_key, status, idempotency_key,
                        request_fingerprint, feed_guid, public_audio_url, attempt_count,
                        created_at, updated_at
                    ) VALUES (
                        :episode_id, 'rss', 'local-rss', 'published', 'target-backfill-key',
                        :fingerprint, 'target-backfill-guid',
                        'https://podcast.example.test/media/immutable.mp3', 2,
                        CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                    )
                    """
                ),
                {"episode_id": episode_id, "fingerprint": "a" * 64},
            )
    finally:
        engine.dispose()
    ...
    try:
        with engine.connect() as connection:
            target = connection.execute(
                text(
                    """
                    SELECT platform, status, remote_id, remote_url, attempt_count
        ...
        assert target == (
            "rss",
            "published",
            "target-backfill-guid",
            "https://podcast.example.test/media/immutable.mp3",
            2,
        )
    finally:
        engine.dispose()
