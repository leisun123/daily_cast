...
class _RecordedAlerts:
    """Capture the (stage, error) pairs the briefing service reports."""

    def __init__(self) -> None:
        self.events: list[tuple[str, str]] = []

    async def alertmsg(self, stage: str, error: Exception) -> None:
        self.events.append((stage, str(error)))
...
def test_briefing_dry_run_generates_full_output_without_any_delivery_side_effect(
    session_factory: sessionmaker[Session], tmp_path: Path
) -> None:
    """A rehearsal produces the merged message but cannot reach the webhook or the 08:30 tick."""
    ...
    report = asyncio.run(service.dry_run())

    statuses = {entry.category: entry.status for entry in report.categories}
    assert statuses == {"telecom": "generated", "ai": "generated"}
    assert report.merged_markdown is not None
    assert "## 📡 通信" in report.merged_markdown
    assert "## 🤖 AI" in report.merged_markdown
    assert "演练用的昨日关注总结。" in report.merged_markdown
    # Nothing may reach the webhook, and no operator alert may fire for a rehearsal.
    assert notifier.pushed == []
    assert alerts.events == []
    # Output is quarantined below the dry-run directory the delivery path never reads.
    assert all(
        entry.file_path is not None and output_dir / "dry-run" in entry.file_path.parents
        for entry in report.categories
    )
    assert (output_dir / "dry-run" / f"{report.date}-merged.md").is_file()
    assert list(output_dir.glob("*.md")) == []
    assert list(output_dir.glob("*.done")) == []
    assert list(output_dir.glob("*-merged.prepared.json")) == []
    # The scheduled 08:30 delivery tick must not send the rehearsal output.
    delivery = asyncio.run(service.deliver_prepared())
    assert all(entry.reason == "not_prepared" for entry in delivery.categories)
    assert notifier.pushed == []
    # The acceptance readout ignores rehearsal files.
...
class RecordingExtractor(ContentExtractor):
    ...
    def __init__(self, content: str, *, published_at: datetime | None = None) -> None:
        self._content = content
        self._published_at = published_at or (datetime.now(UTC) - timedelta(hours=2))
...
def test_verify_skips_articles_fetched_earlier_in_the_same_run(
    ...
    _seed_source(session_factory, "telecom-source", category="telecom")
    _seed_source(session_factory, "ai-source", category="ai")
    # No body on the feed candidate: extraction must fetch, and those successes
    # must stay eligible without a second verification GET.
    collector = FakeRSSCollector(
        {
            "telecom-source": [
                _candidate(
            ...
            "ai-source": [
                _candidate("ai-source", f"a{index}", content_text="", published_at=None)
                for index in range(1, 3)
            ],
        }
    )
...
def test_extract_failure_is_not_refetched_by_verify(
    session_factory: sessionmaker[Session], tmp_path: Path
) -> None:
    """A settled extract failure must not trigger a second full fetch of the same URL."""
    _seed_source(session_factory, "telecom-source", category="telecom")
    _seed_source(session_factory, "ai-source", category="ai")
    good_url = "https://telecom-source.example.test/t1"
    bad_url = "https://telecom-source.example.test/t2"
    collector = FakeRSSCollector(
        {
            "telecom-source": [
                _candidate("telecom-source", "t1", content_text="", published_at=None),
                _candidate("telecom-source", "t2", content_text="", published_at=None),
            ],
            "ai-source": [_candidate("ai-source", "a1")],
        }
    )
    ...
    class SelectiveFailureExtractor(ContentExtractor):
        def __init__(self) -> None:
            self.requested_urls: list[str] = []
        ...
        async def extract(self, url: str, policy: FetchPolicy) -> ExtractedArticle:
            ...
            if url == bad_url:
                return ExtractedArticle(
                    ...
                    error=SourceError(
                        code="HTTP_STATUS",
                        summary="source returned HTTP 403",
                        retryable=False,
                    ),
                )
            return ExtractedArticle(
                requested_url=url,
                final_url=url,
                content_text="抓取的正文内容。" * 10,
                http_status=200,
                fetched_at=datetime.now(UTC),
                published_at=datetime.now(UTC) - timedelta(hours=2),
            )

    extractor = SelectiveFailureExtractor()
    service = _build_service(
        session_factory,
        tmp_path / "briefings",
        collector=collector,
    ...
    assert {entry.status for entry in report.categories} == {"generated"}
    assert extractor.requested_urls.count(bad_url) == 1
    assert extractor.requested_urls.count(good_url) == 1
